...
export class UserIdleSessionTimeoutService implements OnDestroy {
  ...
  public initilizeSessionTimeout(): void {
    UserIdleSessionTimeoutService.runTimer = true;

    if (this.USER_IDLE_TIMER_VALUE_IN_MIN === 0) {
      this.userIdlenessChecker.thrownError('Please provide USER_IDLE_TIMER_VALUE in minute');
      return;
    }
  ...
  private initInterval(): void {
    ...
    this.sessionForIdle = interval(intervalDuration).pipe(
      ...
      takeWhile(() => UserIdleSessionTimeoutService.runTimer)
    );
  ...
  private executeFinalTimer = () => {
    UserIdleSessionTimeoutService.runSecondTimer = true;
  ...
  private initializeFinalTimer(): void {
    ...
    this.clockForIdle = interval(intervalDuration).pipe(
      ...
      takeWhile(() => UserIdleSessionTimeoutService.runSecondTimer)
    );
